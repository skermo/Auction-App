...
(
    ...
    CONSTRAINT PK_bid PRIMARY KEY (item_id, user_id),
...
CREATE TRIGGER update_highest_price
    ...
    FOR EACH ROW EXECUTE FUNCTION update_highest_price();


CREATE OR REPLACE FUNCTION update_bid_number() RETURNS TRIGGER
AS $update_bid_number$
DECLARE
    TG_item_id          uuid;
BEGIN
...
CREATE TRIGGER update_bid_number
    AFTER INSERT OR DELETE ON bid
    FOR EACH ROW EXECUTE FUNCTION update_bid_number();
